...
function getBackendJar() {
  if (app.isPackaged) {
    return path.join(process.resourcesPath, 'backend', 'compraflow.jar');
  }
  return path.resolve(__dirname, '..', 'target', 'compraflow-1.0.0.jar');
}

function normalizeH2Path(filePath) {
...
async function startBackend() {
  ...
  if (app.isPackaged && !fs.existsSync(java)) {
    throw new Error(`Runtime Java não encontrado: ${java}`);
  }
  if (!fs.existsSync(jar)) {
    throw new Error(`Backend CompraFlow não encontrado: ${jar}`);
  }
  ...
  const appendLog = (chunk) => {
    try { fs.appendFileSync(backendLogFile, chunk); } catch (_) {}
  };
...
async function apiRequest({ method = 'GET', endpoint, body }) {
  if (!backendBaseUrl) {
    return { ok: false, status: 503, data: { detail: 'Backend ainda não está disponível.' } };
  }
  try {
